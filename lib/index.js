...
const fillPlaylistDropdown = (playlist) => {
  $(".fav-dropdown").append(`
    <option id="${playist.id}">${playlist.name}</option>
    `)
}
...
const getPlaylists = () => {
  fetch('https://api-play.herokuapp.com/api/v1/playlists')
  .then((response) => response.json())
  .then((playlistsData) => compilePlaylists(playlistsData))
  .catch((error) => console.error({ error }));
}
...
const displayPlaylists = (playlist) => {
  $('.playlists-index-table').append(`
    <tr class="playlists-table-row">
      ...
      <td class="song-cell" id="song-cell-title-"><button class="playlist-title-link">${playlist.name}</button></td>
    </tr>`)
}
